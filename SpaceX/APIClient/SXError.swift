...
struct SXError: Error {
  
  var code: Int
  var codeKey: SXErrorKeys
  var domain: String
  var message: String?
  var userInfo: Any?
  
  init(code: Int,
       codeKey: SXErrorKeys = .UNKNOWN_ERROR,
       domain: String = SXAppContext.bundleID,
       message: String? = nil,
       userInfo: Any? = nil) {
    
    self.code = code
    self.codeKey = SXErrorKeys.errorKey(from: code)
    self.domain = domain
  ...
  init(urlError: URLError) {
    ...
    self.codeKey = SXErrorKeys.errorKey(from: urlError.errorCode)
    self.domain = ""
    self.message = message ?? urlError.localizedDescription
    self.userInfo = urlError.errorUserInfo
  }
  
  init(nsError: NSError) {
    
    self.code = nsError.code
    self.codeKey = SXErrorKeys.errorKey(from: nsError.code)
  ...
  init(error: Error) {
    ...
    self.code = SXErrorKeys.UNKNOWN_ERROR.rawValue
    self.codeKey = SXErrorKeys.UNKNOWN_ERROR
    self.domain = SXAppContext.bundleID
    self.message = error.localizedDescription.capitalized
    self.userInfo = error
  }
  
  static func toError(error: Error) -> SXError {
    
    if let urlError = error as? URLError {
      return SXError(urlError: urlError)
      
    } else if let _error = error as? SXError {
      return _error
    } else {
      return SXError(error: error)
    }
  }
  
  static func unknownError() -> SXError {
    return SXError(code: SXErrorKeys.UNKNOWN_ERROR.rawValue)
  }
}

enum SXErrorKeys: Int, CaseIterable {
...
extension SXErrorKeys {
  
  /// Return converted rawValue to String.
  public var toString: String {
    return String(describing: self)
  }
  
  public static func toRawValue(code: String) -> Int {
    
    var codeValue: Int?
    for type in self.allCases {
      if type.toString == code {
        codeValue = type.rawValue
      }
    }
    
    guard let rawValue = codeValue else {
      return UNKNOWN_ERROR.rawValue
    }
    
    return rawValue
  }
  
  public static func errorKey(from code: Int) -> SXErrorKeys {
    
    var codeKey: SXErrorKeys?
    for type in self.allCases {
      if type.rawValue == code {
        codeKey = type
      }
    }
    
    guard let _codeKey = codeKey else {
